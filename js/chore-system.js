        ...
        app.tutorialData = {
            ...
            steps: [
                ...
                { id: 2, name: 'groupTasks', type: 'tooltip', target: 'groupTaskToggle' },
                { id: 3, name: 'createTask', type: 'modal' },
                { id: 4, name: 'subCategory', type: 'tooltip', target: 'subCategorySelect' },
                { id: 5, name: 'completeTask', type: 'tooltip', target: 'tutorialTask' },
                { id: 6, name: 'loadTaskList', type: 'modal' }
            ]
        };

        app.startUpkeepTutorial = function() {
            console.log('🎓 Starting Upkeep Tutorial...');
            
            // Save current state
            this.tutorialData.originalData = JSON.parse(JSON.stringify(this.data));
            this.tutorialData.isActive = true;
            this.tutorialData.currentStep = 1;
            
            // Create a tutorial category if it doesn't exist
            ...
            this.data.categories.push(tutorialCategory);
            this.tutorialData.tutorialCategoryId = tutorialCategory.id;
            
            // Show tutorial overlay
            const tutorialOverlay = document.getElementById('upkeepTutorial');
            if (tutorialOverlay) {
                tutorialOverlay.style.display = 'block';
                
                // Disable all other UI elements
                this.setTutorialUILocks(true);
        ...
        app.showTutorialStep = function(stepNumber) {
            // Hide all steps
            ...
            if (currentStepEl) {
                ...
                switch(stepNumber) {
                    case 1:
                        // Welcome screen - just show modal
                        break;
                        
                    case 2:
                        // Show dashboard with tutorial category
                        this.data.currentCategoryId = this.tutorialData.tutorialCategoryId;
                        this.render();
                        
                        // Position tooltip near group task toggle
                        setTimeout(() => {
                            this.positionTutorialTooltip('groupTaskTooltip', 'groupTaskToggleBtn');
                            // Add event listener for toggle click
                            const toggleBtn = document.getElementById('groupTaskToggleBtn');
                            if (toggleBtn) {
                                toggleBtn.classList.add('tutorial-spotlight');
                                toggleBtn.onclick = () => {
                                    app.toggleGroupTaskVisibility();
                                    app.tutorialNextStep();
                                };
                            } else {
                                // If no group tasks, auto-advance
                                setTimeout(() => this.tutorialNextStep(), 1000);
                            }
                        }, 500);
                        break;
                    ...
                    case 4:
                        // Sub-category selection (shown when modal opens)
                        break;
                        
                    case 5:
                        // Complete task
                        setTimeout(() => {
                            const taskEl = document.querySelector(`[data-task-id="${this.tutorialData.tutorialTaskId}"]`);
                            if (taskEl) {
                                this.positionTutorialTooltip('completeTaskTooltip', taskEl);
                                taskEl.classList.add('tutorial-spotlight');
                            }
                        }, 500);
                        break;
                        
                    case 6:
                        // Load task list information
                        break;
                }
            }
            
            this.tutorialData.currentStep = stepNumber;
        };

        app.tutorialNextStep = function() {
            const nextStep = this.tutorialData.currentStep + 1;
            
            if (nextStep <= this.tutorialData.totalSteps) {
        ...
        app.tutorialShowAddTaskModal = function() {
            // Show add task modal with tutorial constraints
            this.showAddTaskModal();
            
            // Pre-fill with tutorial requirements
            setTimeout(() => {
                const taskNameInput = document.getElementById('taskName');
                const decayValueInput = document.getElementById('taskDecayValue');
                const decayUnitSelect = document.getElementById('taskDecayUnit');
                
                if (taskNameInput) {
                    taskNameInput.value = 'Tutorial Task';
                    taskNameInput.setAttribute('readonly', 'true');
                }
                if (decayValueInput) {
                    decayValueInput.value = 6;
                }
                if (decayUnitSelect) {
                    decayUnitSelect.value = 'weeks';
                }
                
                // Show sub-category tooltip if in Sweep/Mop/Vacuum category
                const category = this.data.categories.find(c => c.id === this.data.currentCategoryId);
                if (category && category.name === 'Sweep/Mop/Vacuum') {
                ...
                // Override form submission for tutorial
                ...
                if (addTaskForm) {
                    addTaskForm.onsubmit = (e) => {
                        ...
                        const task = {
                            id: Date.now(),
                            name: 'Tutorial Task',
                            decayMs: 6 * 7 * 24 * 60 * 60 * 1000,  // 6 weeks
                            decayUnit: 'weeks',
                            lastCompleted: null,
                            freshness: 0,
                            isTutorial: true
                        ...
                        if (category) {
                            ...
                            this.tutorialData.tutorialTaskId = task.id;
                        }
        ...
        app.positionTutorialTooltip = function(tooltipId, targetElement) {
            ...
            if (tooltip && target) {
                ...
                tooltip.style.left = (rect.left + rect.width/2 - 200) + 'px';  // Center tooltip (200 = half of max-width)
                
                // Add pointing class
                tooltip.classList.add('pointing-down');
                
                // Check if tooltip goes off-screen and adjust
                if (rect.bottom + tooltipHeight + 10 > window.innerHeight) {
                    // Position above instead
                    tooltip.style.top = (rect.top - tooltipHeight - 10) + 'px';
                    tooltip.classList.remove('pointing-down');
                    tooltip.classList.add('pointing-up');
                }
            }
        };

        app.setTutorialUILocks = function(lock) {
            if (lock) {
                // Disable all navigation during tutorial
                ...
                // Disable Battle System navigation
                const dashboardView = document.getElementById('dashboardView');
                if (dashboardView) {
                    const categoryCards = dashboardView.querySelectorAll('.category-card');
                    categoryCards.forEach(card => {
                        if (!card.textContent.includes('Tutorial Category')) {
                ...
                const disabledElements = document.querySelectorAll('.tutorial-disabled');
        ...
        app.completeTutorial = function() {
            ...
            this.data.upkeepTutorialCompleted = true;
            
            // Remove tutorial category and tasks
            this.data.categories = this.data.categories.filter(c => !c.isTutorial);
            ...
            // Re-enable UI
            this.setTutorialUILocks(false);
            
            // Reset tutorial data
            this.tutorialData.isActive = false;
            this.tutorialData.currentStep = 0;
            this.tutorialData.tutorialCategoryId = null;
            this.tutorialData.tutorialTaskId = null;
            
            // Show celebration
            this.showSpeechBubble('🎉 Congratulations! You\'ve mastered the Upkeep system! You\'re ready to keep your home in perfect condition!');
            
            // Refresh dashboard
            this.showDashboard();
        };

        app.skipTutorial = function() {
            if (confirm('Are you sure you want to skip the tutorial? You can always access help from the settings menu.')) {
                this.data.upkeepTutorialCompleted = true;
                this.saveData();
