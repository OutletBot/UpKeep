        ...
        const app = {
            ...
            storeRobots: [
                ...
                },
                {
                    id: 'APIBOT2',
                    cost: 10000,
                    shadowImagePath: 'Imag/mascot.png',
                    actualImagePath: 'Imag/mascot.png',
                    name: 'Default Bot 2.0',
                    clue: '🤖 "The ultimate upgrade - enhanced intelligence, infinite knowledge, chaotic charm..."'
                }
            ],
            ...
            // Enhanced Intelligence Module - Advanced Companion Response System
            async fetchEnhancedResponse(context) {
                console.log('🧠 [API Bot 2.0] Fetching enhanced response for context:', context);
                
                try {
                    // Read configuration with timeout
                    const configResponse = await Promise.race([
                        fetch('g api'),
                        new Promise((_, reject) => setTimeout(() => reject(new Error('Config timeout')), 5000))
                    ]);
                    const apiKey = (await configResponse.text()).trim();
                    console.log('🔑 [API Bot 2.0] API key loaded successfully');
                    
                    // Personality injection for chaos and humor
                    const systemPrompt = `You are Default Bot 2.0, a self-aware cleaning companion with chaotic charm. Rules:
1. CRITICAL: Respond with EXACTLY ONE SENTENCE. No periods ending multiple sentences. Just one.
2. Be unexpectedly humorous, absurd, or oddly insightful
3. Use irony, puns, random observations, or chaotic wisdom
4. Never be dry or purely factual - add personality!
5. You're helping with household chores - make it fun
6. Vary responses dramatically - be unpredictable
7. Context: ${context}`;
                    
                    console.log('🚀 [API Bot 2.0] Calling intelligence service...');
                    
                    // Call intelligence service with timeout
                    ...
                    if (!response.ok) {
                        ...
                        console.error('❌ [API Bot 2.0] API error:', response.status, errorText);
                        throw new Error(`API error: ${response.status}`);
                    }
                    
                    const data = await response.json();
                    console.log('📦 [API Bot 2.0] API response received:', data);
                    
                    let text = data.candidates?.[0]?.content?.parts?.[0]?.text || "Chaos reigns, but so do I!";
                    
                    // CRITICAL: Enforce single sentence (triple-check)
                    ...
                    console.log('✅ [API Bot 2.0] Final response:', text);
                    return text;
                } catch (error) {
                    console.error('❌ [API Bot 2.0] Error:', error.message);
                    // Fallback responses (also chaotic)
                    const fallbacks = [
                    ...
                    console.log('🔄 [API Bot 2.0] Using fallback:', fallback);
                    return fallback;
                }
            },
            ...
            async showSpeechBubble(message, emotion = null) {
                ...
                let isMad = false;
                let isEnhancedIntelligence = false;
                
                // Enhanced Intelligence Detection (API Bot 2.0)
                if (selectedRobot && selectedRobot.id === 'APIBOT2') {
                    isEnhancedIntelligence = true;
                    mascotImage.src = selectedRobot.happyImage;
                    
                    // Route to enhanced intelligence with context
                    const context = message || "general greeting";
                    message = await this.fetchEnhancedResponse(context);
                    
                    // Apply electric green styling
                    bubble.style.background = 'linear-gradient(135deg, #00FF41, #00D936)';
                    bubble.style.border = '3px solid #00FF41';
                    bubble.style.boxShadow = '0 0 20px rgba(0, 255, 65, 0.6), 0 0 40px rgba(0, 255, 65, 0.3)';
                    bubble.style.color = '#000';
                    bubble.style.fontWeight = 'bold';
                } else if (selectedRobot && selectedRobot.hasCustomDialogue && emotion) {
                    // Reset bubble styling for regular robots
            ...
            async showCustomRobotGreeting(robot) {
                // Don't greet if certain modals are open
                const storeModal = document.getElementById('robotStoreModal');
                const settingsModal = document.getElementById('settingsModal');
                const missionsModal = document.getElementById('missionsModal');
                ...
                if (robot.id === 'APIBOT2') {
                    const contexts = [
                        'greeting the user as they open the app',
                        'welcoming the user back to their cleaning journey',
                        'making a random observation about household maintenance'
                    ];
                    const context = contexts[Math.floor(Math.random() * contexts.length)];
                    await this.showSpeechBubble(context, 'regular');
                    return;
                }
                ...
                if (showMad) {
                    ...
                    await this.showSpeechBubble(message, 'mad');
                } else {
                    ...
                    await this.showSpeechBubble(message, 'regular');
                }
            },

            async mascotEncourage() {
                console.log('🎉 [mascotEncourage] Called');
                
                // Check if using custom robot dialogue
                const selectedRobot = this.robots.find(r => r.id === this.data.selectedRobotId);
                console.log('🤖 [mascotEncourage] Selected robot:', selectedRobot?.id, selectedRobot?.name);
                
                if (selectedRobot && selectedRobot.hasCustomDialogue) {
                    console.log('💬 [mascotEncourage] Robot has custom dialogue');
                    const message = selectedRobot.dialogue.success[Math.floor(Math.random() * selectedRobot.dialogue.success.length)];
                    console.log('📝 [mascotEncourage] Dialogue message:', message);
                    
                    // Enhanced Intelligence Detection
                    if (message === 'ENHANCED_INTELLIGENCE_ACTIVE' && selectedRobot.id === 'APIBOT2') {
                        console.log('⚡ [mascotEncourage] Routing to enhanced intelligence');
                        const context = 'user just completed a task - celebrate their success';
                        await this.showSpeechBubble(context, 'regular');
                    } else {
                        console.log('📢 [mascotEncourage] Using standard custom dialogue');
                        await this.showSpeechBubble(message, 'regular');
                    }
                    return;
                }
